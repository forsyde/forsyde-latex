...
\cs_new_protected:Nn \davs__tokens_do:n
 {
	\ifthenelse{ \equal{#1}{scalar}}{
  	...
  	\ifthenelse{ \equal{#1}{scalarAbstExt}}{
		\draw[fill=\defaultfillcolor, thin] (\xTemp,0) circle [radius=\tokensize];
		\pgfsetlinewidth{.5pt} 
	...
	\ifthenelse{ \equal{#1}{vectorAbstExt}}{
		\draw[fill=\defaultfillcolor, thin ] (\xTemp-\tokensize,0) -- (\xTemp,-\tokensize) -- (\xTemp+\tokensize,0) -- (\xTemp,\tokensize) -- cycle;
		\pgfsetlinewidth{.5pt} 
		\pgfpathmoveto{\pgfpoint{\xTemp}{-.75\inTemp}}
		\pgfpathlineto{\pgfpoint{\xTemp}{\inTemp}}
  	...
  	\ifthenelse{ \equal{#1}{functionAbstExt}}{
		\draw[fill=\defaultfillcolor, thin ] (\xTemp-\tokensize,-\tokensize) rectangle (\xTemp+\tokensize,\tokensize);
		\pgfsetlinewidth{.5pt} 
...
\tikzset{
  tok/.style 2 args={
  	decoration={
		...
		post length=1pt,
		pre length=1pt,
    	mark=at position #1 with {\tokens{#2}
    	}
  	}, postaction=decorate },
  ...
  -|-|/.style args = {#1:#2}{ to path={ (\tikztostart) 
	-| ($(\tikztostart)!#1!(\tikztotarget)$) 
	|- ([yshift=#2]\tikztotarget) 
	-| (\tikztotarget) 
	\tikztonodes }
  },
  -|-|/.default=0.5:10pt,
  -|-|-/.style args = {#1:#2:#3}{ to path={ (\tikztostart) 
	-| ($(\tikztostart)!#1!(\tikztotarget)$) 
	|- ([yshift=#3]$(\tikztostart)!#2!(\tikztotarget)$) 
	-| ([yshift=#3]$(\tikztostart)!#2!(\tikztotarget)$)
	|- (\tikztotarget) 
	\tikztonodes }
  },
  -|-|-/.default=0.3:0.7:20pt,
  |-|-|/.style args = {#1:#2:#3}{ to path={ (\tikztostart) 
	|- ($(\tikztostart)!#1!(\tikztotarget)$) 
	-| ([xshift=#3]$(\tikztostart)!#2!(\tikztotarget)$) 
	|- ([xshift=#3]$(\tikztostart)!#2!(\tikztotarget)$)
	-| (\tikztotarget) 
	\tikztonodes }
  },
