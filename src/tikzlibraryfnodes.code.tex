...
% Helper for embedding another node into a cluster
%
% \embed[keys-cluster] [(name-cluster)] {embedded-node}
\NewDocumentCommand\embed{O{} D(){} m}{%
  \begin{scope}[local bounding box=tempInterior]
    #3%
  \end{scope}
  \cluster[#1] (#2) <(tempInterior)> {};
}
