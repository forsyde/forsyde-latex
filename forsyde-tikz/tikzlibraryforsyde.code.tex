...
\pgfkeys{/forsyde keys/positioning/.is family, /forsyde keys/positioning,
  shift/.estore in       = \fsdPosShift,
  relative to/.estore in = \fsdPosRelTo,
  at/.style       = {relative to=#1}
  left of/.style  = {relative to={#1.east}, shift={1,0}},
  right of/.style = {relative to={#1.west}, shift={-1,0}},
  above of/.style = {relative to={#1.north}, shift={0,1}},
  below of/.style = {relative to={#1.south}, shift={0,-1}},
  between/.style args = {#1:#2}{relative to={$(#1)!.5!(#2)$}},
  default/.style = {
    relative to = {0,0},
    shift       = {0,0}
  }
...
% case).
%
% \standard  [keys] (name) <global pos> {};
% \standard* [keys] (name) <relative pos keys>  {};
\NewDocumentCommand\standard{s O{} r() D<>{0,0} m}{
%% \newcommand{\standard}[4][]{
  \pgfkeys{/forsyde keys, default, #2}%
  \IfBooleanTF {#1}%
    { \pgfkeys{/forsyde keys/positioning, default, #4}%
      \coordinate (globalPos) at ($ (\fsdPosShift) + (\fsdPosRelTo) $); }
    { \coordinate (globalPos) at (#4); }
  ...
  \begin{scope}[%
    ...
    \node[] (#3-l) at (0,0) {\labelStyle{#5}};
    % the functions are inside the scope, above the label
    \node[%
      func\fsdNFunctions,
      font=\funcStyle,
      yshift=\fsdInnerYSep,
      anchor=north] (#3-f) at (#3-l.north) {%
